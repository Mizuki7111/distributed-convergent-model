...
def get_args():
    ...
    psr.add_argument("--sim_steps", type=int, default=200)
    psr.add_argument("--num_sim_train", type=int, default=20)
    psr.add_argument("--num_sim_validation", type=int, default=5)
    psr.add_argument("--num_sim_test", type=int, default=5)
    psr.add_argument("--dt", type=float, default=0.01)
    psr.add_argument("--dim_hid", type=int, default=32)
    ...
    psr.add_argument("--lr", type=float, default=2.0e-4)
...
def simulate_aggregation(
    ...
    sim = simulate_data.AggregationStaticGraph()
    data_train = np.zeros((num_sim_train, steps, n, 1))
    graph_train = []
    data_validation = np.zeros((num_sim_validation, steps, n, 1))
    graph_validation = []
    data_test = np.zeros((num_sim_test, steps, n, 1))
    graph_test = []
    for i in range(num_sim_train):
        state_init = nr.normal(0.0, 5.0, (n, 1))
        graph = simulate_data.random_graph(n, 0.5)
        data_train[i] = sim(steps, state_init, dt, stable_graph=graph)
        graph_train.append(graph)
    for i in range(num_sim_validation):
        state_init = nr.normal(0.0, 5.0, (n, 1))
        graph = simulate_data.random_graph(n, 0.5)
        data_validation[i] = sim(steps, state_init, dt, stable_graph=graph)
        graph_validation.append(graph)
    for i in range(num_sim_test):
        state_init = nr.normal(0.0, 5.0, (n, 1))
        graph = simulate_data.random_graph(n, 0.5)
        data_test[i] = sim(steps, state_init, dt, stable_graph=graph)
...
def learning_models(args, save_dir):
    ...
    if args.save_all_files:
        ...
        with open(f"{save_dir}/graph_train.pkl", "wb") as f:
            pickle.dump([g.tolist() for g in graph_train], f)
        np.save(f"{save_dir}/data_validation.npy", data_validation)
        with open(f"{save_dir}/graph_validation.pkl", "wb") as f:
            pickle.dump([g.tolist() for g in graph_validation], f)
    ...
    model_dim = 1
    model_dist = 1.0
    data_train = datasets.StaticGraphDataset(data_train, graph_train, args.dt)
    ...
    logger.debug(
        f"v_pairwise->PairwiseFunction: {model_dim}, {args.dim_hid}, 1, {args.num_layers}, 'models.DistanceGraphCofficient({model_dist}, {model_dim})'"
    )
    ...
    test_v_pair_list = []
    for vp in projection_model.v.pair_funcs:
        test_v_pair_list.append(test_models.test_v_pair(vp, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/projection/final_v_pair.npy", test_v_pair_list, allow_pickle=True
    )
    test_v_each_list = []
    for ve in projection_model.v.each_funcs:
        test_v_each_list.append(test_models.test_v_self(ve, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/projection/final_v_each.npy", test_v_each_list, allow_pickle=True
    )
    ...
    for vp in projection_model.v.pair_funcs:
        test_v_pair_list.append(test_models.test_v_pair(vp, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/projection/best_v_pair.npy", test_v_pair_list, allow_pickle=True
    )
    test_v_each_list = []
    for ve in projection_model.v.each_funcs:
        test_v_each_list.append(test_models.test_v_self(ve, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/projection/best_v_each.npy", test_v_each_list, allow_pickle=True
    )
    ...
    hamiltonian_model = models.HamiltonianModel(
        j_mat=j_mat, r_mat=r_mat, v=v_pro, dim=1
    )
    logger.debug(summary(hamiltonian_model, verbose=0))
    logger.debug("Args of Hamiltonian model")
    logger.debug(
        f"v_pairwise->PairwiseFunction: {model_dim}, {args.dim_hid}, 1, {args.num_layers}"
    )
    logger.debug(f"v_each -> MLP: {model_dim}, {args.dim_hid}, 1, {args.num_layers}")
    logger.debug(f"v_each -> QuadraticFunction")
    logger.debug(
    ...
    test_v_pair_list = []
    for vp in hamiltonian_model.v.pair_funcs:
        test_v_pair_list.append(test_models.test_v_pair(vp, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/hamiltonian/final_v_pair.npy", test_v_pair_list, allow_pickle=True
    )
    test_v_each_list = []
    for ve in hamiltonian_model.v.each_funcs:
        test_v_each_list.append(test_models.test_v_self(ve, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/hamiltonian/final_v_each.npy", test_v_each_list, allow_pickle=True
    )
    ...
    for vp in hamiltonian_model.v.pair_funcs:
        test_v_pair_list.append(test_models.test_v_pair(vp, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/hamiltonian/best_v_pair.npy", test_v_pair_list, allow_pickle=True
    )
    test_v_each_list = []
    for ve in hamiltonian_model.v.each_funcs:
        test_v_each_list.append(test_models.test_v_self(ve, V_SAVE_RANGE, 1000))
    np.save(
        f"{save_dir}/hamiltonian/best_v_each.npy", test_v_each_list, allow_pickle=True
    )
...
if __name__ == "__main__":
    ...
    log_config["handlers"]["file_handler"][
        "filename"
    ] = f"{args.log_dir}/aggregation.log"
